# Open Book Research: 开启自启功能 (Launch at Login)
...
1. 创建 `LaunchAtLoginManager.swift` - 管理开启自启逻辑
2. 在 `ThemeSettingsView.swift` 添加 Toggle 开关
3. App 启动时根据默认设置自动启用（默认开启）
4. 状态持久化到 `UserDefaults`
