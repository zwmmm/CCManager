...
- GitHub Actions release workflow 会构建 DMG、CLI、`CCManager.app.zip`，并生成用于自定义更新器的 `appcast.xml`
- 需要确保没有未提交的更改
...
这个文件的内容会用于：
- GitHub Release 的详情
- 自定义更新弹窗中的 Markdown 版本说明
- release workflow 生成 `appcast.xml` 时嵌入 `<description><![CDATA[...]]></description>`
